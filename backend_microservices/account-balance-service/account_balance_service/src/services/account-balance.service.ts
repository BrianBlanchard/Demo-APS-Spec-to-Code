...
export class AccountBalanceService implements IAccountBalanceService {
  constructor(
    private readonly repository: IAccountBalanceRepository,
  ...
  async getBalance(accountId: string): Promise<AccountBalance> {
    ...
    const balance: AccountBalance = {
      ...
      availableCashCredit:
        Number(entity.cash_credit_limit) -
    ...
    if (request.isDebit) {
      ...
      if (request.amount > availableCredit) {
        ...
        throw new ConflictError(
          `Insufficient credit. Available: $${availableCredit.toFixed(2)}, Requested: $${request.amount.toFixed(2)}`
        );
      }
    }

    const { previous, current } = await this.repository.updateBalance(
      accountId,
      request.amount,
      request.isDebit,
